...
// Model for LyitDisplayboard entity
    models.LyitDisplayboard = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/LYIT-FlightCheckerBoardWeb/webresources/ie.lyit.project.checkerboard.entity.lyitdisplayboard/",
        idAttribute: 'id',
        defaults: {
            flightNo: "",
            route: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/LYIT-FlightCheckerBoardWeb/webresources/ie.lyit.project.checkerboard.entity.lyitdisplayboard/';
            }
    ...
    // Collection class for LyitDisplayboard entities
    models.LyitDisplayboardCollection = Backbone.Collection.extend({
        model: models.LyitDisplayboard,
        url: "http://localhost:8080/LYIT-FlightCheckerBoardWeb/webresources/ie.lyit.project.checkerboard.entity.lyitdisplayboard/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.LyitDisplayboardCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-lyitdisplayboard-list-itemis template identifier for item
                        templateName: '#tpl-lyitdisplayboard-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.lyitdisplayboard = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.lyitdisplayboard,
                    // tpl-lyitdisplayboard-details is template identifier for chosen model element
                    templateName: '#tpl-lyitdisplayboard-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.LyitDisplayboard();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-lyitdisplayboard-details is a template identifier for chosen model element
                templateName: '#tpl-lyitdisplayboard-details',
